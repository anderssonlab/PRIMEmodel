...
# Usage message
usage() {
    echo "Usage: $0 --config <config_file> [-1] [-2] [-3] [-4] [-5] [-6] [--predict] [--predictFocal] [--keeptmp] [--numCores <numCores>]"
    echo "  --config            Specify the bash configuration file to use (required for all commands)"
    echo "  -0                  Run the check for input files for predictFocal"
    ...
    echo "  --predict           Run all steps"
    echo "  --predictFocal      Run only steps 0, 4, and 5 as ctss and tc rds objects are provided"
    echo "  --keeptmp           Keep temporary files created in steps 3-5, only if running predict and predictFocal"
    echo "  --numCores         Number of cores to use for parallel processing"
    exit 1
}
...
while [[ "$#" -gt 0 ]]; do
    case $1 in
        ...
        --predictFocal) predictFocal=true ;;
...
ARGS=""
if [ -n "$numCores" ]; then
  ARGS="$ARGS -p $numCores"
fi

# If --predict is specified, add all steps 1-6
if $predict; then
    steps=("1" "2" "3" "4" "5" "6")
fi

# If --predictFocal is specified, add only steps 0, 3-5
if $predictFocal; then
    steps=("0" "4" "5")
fi
...
for step in "${steps[@]}"; do
    case $step in
        ...
        2)
            ...
            ;;
        3)
            # Create /PRIMEmodel_tmp directory before running step 3
            #create_tmp_dir
        ...
        4)
            ...
            if [ "$predictFocal" = true ]; then
                # If running predictFocal, use the provided ctss_rse and region
                CTSS_RSE="$CTSS_RSE_RDS"
                REGION="$REGION_RDS"
            elif [ "$predict" = true ]; then
                # If running predict, use the files from the previous steps
