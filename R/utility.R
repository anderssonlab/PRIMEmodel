...
setup_tmp_dir <- function(output_dir) {
  tmp_dir <- file.path(output_dir, "PRIMEmodel_tmp")
...
setup_log_target <- function(log, outdir) {
  if (is.null(log)) {
    ...
    log_dir <- file.path(outdir, "PRIMEmodel_log")
    ...
    log_file <- file.path(log_dir, "PRIMEmodel.log")
...
## Logging Functions for PRIMEmodel
...
#' to be used in the PRIMEmodel pipeline.
...
prep_profile_dir <- function(output_dir = ".",
  ...
  # Create the output dir and main dir (previously at PRIMEmodel_tmp)
...
batch_heatmapData <- function(regions,
  ...
  profile_list <- lapply(seq_along(region_chunks), function(i) {
    r <- region_chunks[[i]]
    suppressMessages(PRIME::heatmapData(r, data, sparse = sparse, ...))
  })
...
#' for a single chromosome.
#'
#' Internal function used by PRIMEmodel to compute normalized
