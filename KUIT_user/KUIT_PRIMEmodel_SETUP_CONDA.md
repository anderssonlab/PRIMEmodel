# PRIMEmodel Environment Setup Guide
...
```bash
## Change path to PRIMEmodel directiry before running this command
conda install -y -c conda-forge --update-specs --file /PATH/TO/PRIMEmodel/inst/envfile/environment.txt
```
...
## 11. Install PRIMEmodel from local tarball

```bash
R -q -e 'Sys.setenv(RETICULATE_PYTHON="~/.conda/envs/prime-conda-env/bin/python3", PYTHONNOUSERSITE="1"); install.packages("/PATH/TO/PRIMEmodel/PRIMEmodel_1.0.tar.gz", repos=NULL, type="source", lib=.libPaths()[1]); library(PRIMEmodel); packageVersion("PRIMEmodel"); library(reticulate); print(py_config()); cat("\nPRIMEmodel loaded OK ✅\n")'
```

**Expect:**

- PRIME installs cleanly.
- `packageVersion("PRIMEmodel")` prints `1.0`.
- `py_config()` shows Python from `prime-conda-env`.
- Final message: `PRIMEmodel loaded OK ✅`.

---



## 12. Quick PRIMEmodel test
...
library(PRIMEmodel)
packageVersion("PRIMEmodel")

plc_focal_example <- PRIMEmodel::predictFocalExample(python_path = "~/.conda/envs/prime-conda-env/bin/python3")
plc_example <- PRIMEmodel::predictExample(python_path = "~/.conda/envs/prime-conda-env/bin/python3")
```
